...
def triton_(in_ptr0, out_ptr0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp0 = tl.load(in_ptr0 + ((196*x0) + (12544*(r2 // 196)) + (401408*x1) + (r2 % 196)), rmask & xmask, eviction_policy='evict_last', other=0)
        _tmp1 = tl.where(rmask & xmask, _tmp1 + tmp0, _tmp1)
    tmp1 = tl.sum(_tmp1, 1)[:, None]
...
def triton_(in_ptr0, out_ptr0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp0 = tl.load(in_ptr0 + ((28*(((r2 + (7168*x1)) // 28) % 28)) + (784*x0) + (25088*(((r2 + (7168*x1)) // 784))) + (r2 % 28)), rmask & xmask, eviction_policy='evict_last', other=0)
        _tmp1 = tl.where(rmask & xmask, _tmp1 + tmp0, _tmp1)
    tmp1 = tl.sum(_tmp1, 1)[:, None]
    tl.store(out_ptr0 + x3, tmp1, xmask)
''')
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf0 = empty_strided((64, 512), (512, 1), device='cuda', dtype=torch.float32)
        ...
        triton__0.run(tangents_1, buf2, 10, 64, grid=grid(10), stream=stream0)
        del tangents_1
        buf3 = empty_strided((64, 3136), (3136, 1), device='cuda', dtype=torch.float32)
        ...
        triton__1.run(buf0, buf5, 512, 64, grid=grid(512), stream=stream0)
        del buf0
        buf6 = empty_strided((64, 64, 14, 14), (12544, 196, 14, 1), device='cuda', dtype=torch.float32)
        triton__2.run(relu_1, getitem_3, buf3, buf6, 802816, grid=grid(802816), stream=stream0)
        del buf3
        del getitem_3
        del relu_1
        buf7 = empty_strided((64, 2), (1, 64), device='cuda', dtype=torch.float32)
        triton__3.run(buf6, buf7, 128, 6272, grid=grid(128), stream=stream0)
        ...
        del primals_3
        buf10 = buf9[0]
        assert_size_stride(buf10, (64, 32, 14, 14), (6272, 196, 14, 1))
        buf11 = buf9[1]
        assert_size_stride(buf11, (64, 32, 3, 3), (288, 9, 3, 1))
        del buf9
        buf12 = empty_strided((64, 32, 28, 28), (25088, 784, 28, 1), device='cuda', dtype=torch.float32)
        triton__5.run(relu, getitem_1, buf10, buf12, 1605632, grid=grid(1605632), stream=stream0)
        del buf10
        del getitem_1
        del relu
        buf13 = empty_strided((32, 7), (1, 32), device='cuda', dtype=torch.float32)
        triton__6.run(buf12, buf13, 224, 7168, grid=grid(224), stream=stream0)
        buf14 = empty_strided((32, ), (1, ), device='cuda', dtype=torch.float32)
        triton__7.run(buf13, buf14, 32, 7, grid=grid(32), stream=stream0)
        del buf13
...
if __name__ == "__main__":
    ...
    relu = rand_strided((64, 32, 28, 28), (25088, 784, 28, 1), device='cuda:0', dtype=torch.float32)
    getitem = rand_strided((64, 32, 14, 14), (6272, 196, 14, 1), device='cuda:0', dtype=torch.float32)
    getitem_1 = rand_strided((64, 32, 14, 14), (6272, 196, 14, 1), device='cuda:0', dtype=torch.int64)
    relu_1 = rand_strided((64, 64, 14, 14), (12544, 196, 14, 1), device='cuda:0', dtype=torch.float32)
    getitem_3 = rand_strided((64, 64, 7, 7), (3136, 49, 7, 1), device='cuda:0', dtype=torch.int64)
    view = rand_strided((64, 3136), (3136, 1), device='cuda:0', dtype=torch.float32)
    addmm = rand_strided((64, 512), (512, 1), device='cuda:0', dtype=torch.float32)
