...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton__0.run(buf1, primals_2, 1204224, grid=grid(1204224), stream=stream0)
        del primals_2
        buf2 = empty_strided((48, 32, 14, 14), (6272, 196, 14, 1), device='cuda', dtype=torch.float32)
        buf3 = empty_strided((48, 32, 14, 14), (6272, 196, 14, 1), device='cuda', dtype=torch.int64)
        triton__1.run(buf1, buf2, buf3, 301056, grid=grid(301056), stream=stream0)
        buf4 = aten.convolution(buf2, primals_3, None, (1, 1), (1, 1), (1, 1), False, (0, 0), 1)
        assert_size_stride(buf4, (48, 64, 14, 14), (12544, 196, 14, 1))
        buf5 = buf4; del buf4  # reuse
        triton__2.run(buf5, primals_4, 602112, grid=grid(602112), stream=stream0)
        del primals_4
        buf6 = empty_strided((48, 64, 7, 7), (3136, 49, 7, 1), device='cuda', dtype=torch.int64)
        triton__3.run(buf5, buf6, 150528, grid=grid(150528), stream=stream0)
        buf7 = empty_strided((48, 3136), (3136, 1), device='cuda', dtype=torch.float32)
        triton__4.run(buf5, buf7, 150528, grid=grid(150528), stream=stream0)
        buf8 = empty_strided((48, 512), (512, 1), device='cuda', dtype=torch.float32)
        extern_kernels.addmm(primals_6, buf7, as_strided(primals_5, (3136, 512), (1, 3136)), alpha=1, beta=1, out=buf8)
        del primals_6
        buf9 = empty_strided((48, 10), (10, 1), device='cuda', dtype=torch.float32)
